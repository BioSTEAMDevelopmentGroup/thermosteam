...
from ..base import TDependentModel
from math import e, exp, log10
from .utils import N_A, to_num, ws_to_zs, horner, CASDataReader
from .identifiers import pubchem_db
from scipy.interpolate import interp1d
import pandas as pd


__all__ = ['conductivity', 'LaliberteDensityModel', 'LaliberteHeatCapacityModel', 
           'LalibertePartialViscosityModel', 'Laliberte_data', 'Laliberte_water_viscosity', 
           'Laliberte_water_density',  'LaliberteDensityModel', 'Laliberte_water_heat_capacity', 
           'LaliberteHeatCapacityModel', 'dilute_ionic_conductivity', 
...
Laliberte_water_viscosity = TDependentModel(Laliberte_water_viscosity, Tmin=0, Tmax=10e6)

def LalibertePartialViscosityModel(w_w, v1, v2, v3, v4, v5, v6, Tmin, Tmax):
    r'''Return a TDependentModel object that calculates the viscosity of a solute (Pa*s) at arbitrary temperatures (K) using the form proposed by [1]_
    ...
    .. [1] Laliberte, Marc. "A Model for Calculating the Heat Capacity of
       ...
       doi:10.1021/je8008123
    '''
    def Laliberte_partial_viscosity(T):
        t = T-273.15
        mu_i = exp((v1*(1-w_w)**v2 + v3)/(v4*t+1))/(v5*(1-w_w)**v6 + 1)
        return mu_i/1000.
    return TDependentModel(Laliberte_partial_viscosity, Tmin, Tmax)
...
#        doi:10.1021/je8008123
#     '''
#     mu_w = water_viscosity_model.evaluate(T)*1000.
#     w_w = 1 - sum(ws)
#     mu = mu_w**(w_w)
...
def Laliberte_water_density(T):
    ...
    rho_w = (((((-2.8054253E-10*t + 1.0556302E-7)*t - 4.6170461E-5)*t - 0.0079870401)*t + 16.945176)*t + 999.83952) \
        / (1 + 0.01687985*t)
    return rho_w
Laliberte_water_density = TDependentModel(Laliberte_water_density, Tmin=0, Tmax=10e6)

def LaliberteDensityModel(w_w, c0, c1, c2, c3, c4, Tmin, Tmax):
    r'''Return a TDependentModel object that calculates the density of a solute at arbitrary temperatures (K) using the form proposed by Laliberte [1]_.
    ...
    .. [1] Laliberte, Marc. "A Model for Calculating the Heat Capacity of
       ...
       doi:10.1021/je8008123
    '''
    def Laliberte_density(T):
        t = T - 273.15
        return ((c0*(1 - w_w)+c1)*exp(1E-6*(t + c4)**2))/((1 - w_w) + c2 + c3*t)
    return TDependentModel(Laliberte_density, Tmin, Tmax)
...
Laliberte_water_heat_capacity = TDependentModel(Laliberte_water_heat_capacity, Tmin=0, Tmax=10e6)

def LaliberteHeatCapacityModel(w_w, a1, a2, a3, a4, a5, a6, Tmin, Tmax):
    r'''Return a TDependentModel object that calculates the heat capacity (J/kg/K) at arbitrary temperatures (K) of a solute using the form proposed by [1]_.
    ...
    .. [1] Laliberte, Marc. "A Model for Calculating the Heat Capacity of
       ...
       doi:10.1021/je8008123
    '''
    def Laliberte_heat_capacity(T):
        t = T - 273.15
        alpha = a2*t + a3*exp(0.01*t) + a4*(1. - w_w)
        return 1000. * (a1*exp(alpha) + a5*(1. - w_w)**a6)
    return TDependentModel(Laliberte_heat_capacity, Tmin, Tmax)
