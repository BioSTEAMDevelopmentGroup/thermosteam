...
folder = os.path.join(os.path.dirname(__file__), 'Data/Reactions')
...
def Hf_g(CASRN, AvailableMethods=False, Method=None):
    ...
    def list_methods():
        ...
        if CASRN in _TRC_gas and not np.isnan(TRC_gas_data.at[CASRN, 'Hf']):
            methods.append(TRC)
        methods.append(NONE)
        return methods
    if AvailableMethods:
    ...
    if Method == ATCT_G:
        ...
        _Hfg = float(_TRC_gas.at[CASRN, 'Hf'])
    elif Method == NONE:
        return None
    else:
        raise Exception('Failure in in function')
